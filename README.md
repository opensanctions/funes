...
Funes turns web pages into inspection-brief-scoped people/position facts. It orchestrates web capture (via the in-process [Pravda](https://github.com/opensanctions/pravda) library), LLM extraction, and structured storage.
...
Inputs are YAML seed catalogues. Each file's stem names a **Dataset**, which defines the `people_sought` and the prompt-facing `subject_label`. Its **Subjects** name the entities that scope the search and list their initial URLs. For example, `People sought: Heads of international organizations` and `Organization: Global Environment Facility` form one inspection brief. A **URL** is a global identity, deduplicated across subjects. A **Candidate** links one subject to one URL and is the pipeline's unit of work. Subjects may have no seed URLs so that future discovery can populate them.
...
Every completed attempt records exactly one **SnapshotAssessment** — the judgement of the snapshot itself: `usable` or `broken`. Snapshot usability is brief-independent. Broken is explicit and routed to a repair queue that the documented worker below does not consume (it listens on the inspect queue only); brokenness is never attached to a URL, and it produces no inspection.

A usable assessment additionally gets exactly one brief-relative **Inspection**: `hit` (the brief is satisfied; extracted persons and their positions are attached) or `miss` (nothing on the page satisfies the brief, with a reason). Hits are revisited after an interval; misses are not normally retried.
...
uv run --env-file .env procrastinate worker --queues inspect  # consume the inspect queue (-c/--concurrency N); the repair queue stays pending
```
...
`migrate` applies two Alembic ledgers to the shared Postgres database — Pravda's, shipped with the `opensanctions-pravda` package, and Funes's (`funes/migrations/`), which tracks Funes's tables plus the Procrastinate job-queue schema vendored at the pinned Procrastinate version. Bumping Procrastinate means vendoring its upgrade SQL in a new Funes revision. It then bootstraps the catalogue from YAML files under `INPUT_BASE_PATH`: each file becomes a dataset row named after its filename stem, with subjects and global URLs linked through candidates. This seed import is append-only and never updates or deletes existing records. Changing a stored dataset's `people_sought` or `subject_label` fails loudly because it changes the meaning of all its candidates.
